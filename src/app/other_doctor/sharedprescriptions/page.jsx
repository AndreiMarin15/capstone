...
import { attendingPatients } from "@/backend/shared_prescriptions/attending_patients";
...
export default function MyComponent() {
  ...
  const fetchData = async () => {
    const patients = await attendingPatients.getPatients();
  ...
  const filterAndSortPatients = (
    ...
    if (selectedGenders.length > 0) {
      filteredPatients = filteredPatients.filter((patient) =>
        selectedGenders.includes(
          patient.personal_information.gender.toLowerCase()
        )
      );
    }
    ...
    const nameSortFunction = (a, b) => {
      const nameA =
        `${a.personal_information.first_name} ${a.personal_information.last_name}`.toLowerCase();
      const nameB =
        `${b.personal_information.first_name} ${b.personal_information.last_name}`.toLowerCase();
    ...
    if (sortOptionAge) {
      const ageSortFunction = (a, b) => {
        if (sortOptionAge === "youngest") {
          return (
            computeAge(a.personal_information.birthdate) -
            computeAge(b.personal_information.birthdate)
          );
    ...
    setNavigation(
      filteredPatients.map((patient) => ({
        ...
        href: `/other_doctor/sharedprescriptions/about_patient/${patient.id}`,
                ...
                <div className="self-stretch flex items-stretch justify-between gap-2.5">
                  <span className="flex items-stretch justify-between gap-2 py-2 rounded-md border-[0.5px] border-solid border-black">
                    ...
                    <input
                      ...
                      className=" text-sm leading-5 my-auto outline-none"
