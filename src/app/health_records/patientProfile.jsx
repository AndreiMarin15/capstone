...
export default function PatientProfile({ photo, name, age, gender, patientId }) {
			...
			<div className="flex items-center justify-between w-[100%]">
				...
				<div className="flex items-center gap-3.5">
					...
					<span className="flex-col items-stretch">
						<div className="text-black text-xs font-semibold leading-5">{name}</div>
						<div className="flex items-center">
							<div className="text-black text-xs leading-5 whitespace-nowrap pr-2">{age} years old</div>
							<div className="relative">
								<div className="bg-stone-300 w-px h-4 top-0 bottom-0 left-1/2 transform -translate-x-1/2" />
							</div>
							<div className="text-black text-xs leading-5 whitespace-nowrap pl-2">{gender}</div>
						</div>
					</span>
				</div>
