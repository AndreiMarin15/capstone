...
import { getFamilyAndSocialHistoryDoctor } from "../../../../../../lib/backend/patient/personal_details/master_data";
export default function SocialHistory({ patientId }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const familyAndSocialHistory =
          await getFamilyAndSocialHistoryDoctor(patientId);
    ...
    <>
      ...
      {currentPage === 1 ? (
        <>
          <FamilyHistory
            data={currentFamily}
            setCurrentPage={handleBackClick}
          />
        </>
      ) : (
        ""
