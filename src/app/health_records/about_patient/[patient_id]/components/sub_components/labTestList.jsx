    ...
    async function fetchEncountersAndObservations() {
      try {
        ...
        const labTestObservations = observationsData
          .filter(
            (observation) =>
              newContainedIDs.includes(observation.id) &&
      ...
      if (observation !== undefined && observation !== null) {
        ...
        if (Array.isArray(savedData) && savedData.length > 0) {
          ...
          if (encounterToUpdate) {
            ...
            await updateEncounterContained(
              encounterToUpdate.resource.contained,
              encounterToUpdate
            );
        ...
        <>
          <span className="flex max-w-full justify-between gap-5 items-start max-md:flex-wrap">
            ...
            <div className="flex aspect-[3.3333333333333335] flex-col justify-center items-stretch mt-1.5">
              <span className="flex gap-1.5 justify-between px-10 py-1 rounded border border-blue-800 text-blue-800 border-solid text-sm font-semibold border-1.5">
                <button
                  onClick={() => {
                    setTest(false);
                    setAdd(true);
          ...
          {labTests.length === 0 ? ( // Check if labTests array is empty
            <div className="text-black text-base font-medium mt-4">
            ...
            labTests?.map((item) => (
              <button
                ...
                className={`flex flex-col mt-8 ${
                  item.status === "requested" ? "cursor-not-allowed" : ""
                }`}
                key={item.variable}
                disabled={item.status === "requested"}
                style={{
                  pointerEvents: item.status === "requested" ? "none" : "auto",
                }}
              >
                ...
                <span className="flex items-center gap-3 ml-8 mt-1 self-start w-full">
                  ...
                  {item.status === "requested" && (
                    <div className="text-black text-sm font-medium leading-5 flex items-center">
                      <svg
