...
import { getHeartRateDoctor } from "@/app/lib/backend//patient/vitalsAndBiometrics/vitalsAndBiometrics";
import "chartjs-adapter-date-fns";
import Image from "next/image";

ChartJS.register(TimeScale, LinearScale, PointElement, LineElement);

export default function ViewHeartRate({ currentPage, setCurrentPage, patientId }) {
	// Sample data
	useEffect(() => {
		const fetchData = async () => {
			const hr = await getHeartRateDoctor(patientId);
			console.log(hr);
			setSampleData(hr);
		};
		fetchData();
