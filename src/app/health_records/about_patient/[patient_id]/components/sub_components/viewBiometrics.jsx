...
export default function ViewBiometrics({ currentPage, setCurrentPage, defaultMetric, chartValues, renderingOptions }) {
	...
	useEffect(() => {
        // Ensure selectedMetric is defined before accessing chartValues[selectedMetric]
        if (selectedMetric && chartValues[selectedMetric]) {
            setData({
                labels: chartValues[selectedMetric].slice(-renderingOptions).map(({ date }) => new Date(date)),
                datasets: [
                    {
                        label:
                            selectedMetric === "bmi"
                        ...
                        data: chartValues[selectedMetric].slice(-renderingOptions).map(({ date, value }) => ({
                            x: new Date(date),
                            y: value,
                        })),
                        borderColor: "#0000FF",
    ...
    }, [selectedMetric, chartValues, renderingOptions]);
