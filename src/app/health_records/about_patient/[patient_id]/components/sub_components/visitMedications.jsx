...
export default function VisitMedications({ currentScreen, setCurrentScreen }) {
	const medications = [
		{
			srcmedicine:
	...
	const [currentScreen2, setCurrentScreen2] = useState(0);
		...
		<>
			{currentScreen2 === 0 ? (
				<>
					...
					<div className="text-start text-sm mt-7 whitespace-nowrap font-semibold text-black">Medications</div>
					...
					{medications?.map((medication, index) => (
						<div key={index} className="flex flex-col mt-5 items-start text-xs leading-5 text-black max-w-[701px]">
							...
							<div className="flex gap-5 justify-between self-stretch ml-7 mt-6 w-full font-semibold max-md:flex-wrap max-md:max-w-full">
								<div className="pr-8">
									<span className="">Form</span>: <span className="font-medium">{medication.form}</span>
								</div>
								<div className="pr-8">
									<span className="">Dose</span>: <span className="font-medium">{medication.dose}</span>
								</div>
								<div className="pr-8">
									<span className="">Unit</span>: <span className="font-medium">{medication.unit}</span>
								</div>
								<div className="flex-auto">
									<span className="">Frequency</span>: <span className="font-medium">{medication.frequency}</span>
								</div>
								<div className="flex-auto">
									<span className="">Until</span>: <span className="font-medium">{medication.until}</span>
								</div>
							</div>
						</div>
					))}
