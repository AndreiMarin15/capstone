...
import { getBPDoctor } from "@/backend//patient/vitalsAndBiometrics/vitalsAndBiometrics";

ChartJS.register(TimeScale, LinearScale, PointElement, LineElement);

export default function ViewSystolic({
  currentPage,
  setCurrentPage,
  patientId,
  chartValues,
  ...
  const systolicData = chartValues.systolic
  ...
  const diastolicData = chartValues.diastolic
    .slice(-renderingOptions)
    .map(({ value, date }) => ({
      x: new Date(date),
      y: value,
  ...
  // Table data
  const tableData = chartValues.systolic
    .slice(-renderingOptions)
    .map(({ value: systolic, date }, index) => ({
      date,
      systolic,

      diastolic: chartValues.diastolic.slice(-renderingOptions)[index]?.value,
      commonFormat: `${systolic}/${
        chartValues.diastolic.slice(-renderingOptions)[index]?.value
      } mm(Hg)`,
    }));
      ...
      <div className="max-w-fit text-black">
        ...
        <div className="flex items-center mr-40 mt-5">
          <LineChart
            data={data}
            labels={chartValues.systolic.map(({ date }) => new Date(date))}
            tooltipContent={systolicTooltipContent}
          />
        </div>
      </div>
      <div
        ...
        <div className="overflow-auto">
          <table className="min-w-full ">
            ...
            <tbody>
              {tableData
                ?.reverse()
                ?.map(({ date, systolic, diastolic, commonFormat }) => (
                  <tr key={date}>
