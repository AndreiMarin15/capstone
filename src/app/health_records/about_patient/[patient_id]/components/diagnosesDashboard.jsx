...
import { getDoctorSpecialization, getDoctorHospital } from "@/backend/pdfBackend/getPDFData";

export default function Diagnoses({ patientId }) {
	const variables = ["Diagnoses", "Date of Diagnosis", "Doctor", "Specialization", "Hospital"];
	const [finalDiagnoses, setFinalDiagnoses] = React.useState([]);
	const [encounters, setEncounters] = React.useState([]);

	async function updateDiagnoses(allFinalDiagnoses) {
		const promises = allFinalDiagnoses.map(async (diagnosis) => {
			...
			// Provide default values if the promises resolve to undefined
			return {
				...diagnosis,
	...
	React.useEffect(() => {
		async function fetchFinalDiagnoses() {
			try {
				...
				// Assuming there's code here to fetch allFinalDiagnoses based on patientId
				const updatedDiagnoses = await updateDiagnoses(allFinalDiagnoses);
        console.log("Updated Diagnoses:", updatedDiagnoses);
		...
		async function findEncounters() {
			try {
				// Assuming similar logic to fetch and process encounters
				// Example:
				// const encounters = await fetchEncounters(patientId);
				// process and update state with encounters
			} catch (error) {
