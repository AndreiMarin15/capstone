			...
			{medications.map((medication, index) => (
				<div key={index} className="flex flex-col mt-10 items-start text-xs leading-5 text-black max-w-[1000px]">
					...
					<div className="flex gap-5 justify-between self-stretch ml-12 mt-6 w-full font-semibold max-md:flex-wrap max-md:max-w-full">
						<div className="pr-8">
							<span className="font-medium">Form</span>: <span className="">{medication.form}</span>
						</div>
						<div className="pr-8">
							<span className="font-medium">Dose</span>: <span className="">{medication.dose}</span>
						</div>
						<div className="pr-8">
							<span className="font-medium">Unit</span>: <span className="">{medication.unit}</span>
						</div>
						<div className="flex-auto">
							<span className="font-medium">Frequency</span>: <span className="">{medication.frequency}</span>
						</div>
						<div className="flex-auto">
							<span className="font-medium">Until</span>: <span className="">{medication.until}</span>
						</div>
						<div className="flex-auto">
							<span className="font-medium">Possible Side Effects</span>: <span className="">{medication.side}</span>
						</div>
					</div>
				</div>
			))}
