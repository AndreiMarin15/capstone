...
export const usePatientInfo = create(
	persist(
		(set) => ({
			...
			setMainPractitioner: (item) => set((state) => ({ handled_by: { ...state.handled_by, main_practitioner: item } })),
			addReferredPractitioner: (item) =>
				set((state) => ({
					handled_by: {
						...state.handled_by,
						referred_practitioners: [...state.handled_by.referred_practitioners, item],
					},
				})),
