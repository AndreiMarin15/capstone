	...
	React.useEffect(() => {
		async function fetchEncountersAndObservations() {
			try {
				...
				const conditionObservation = filteredObservationData.find((observation) => observation.id === "condition");
							...
							<div className="flex gap-[4rem] align-baseline">
								<table className="max-w-fit border-spacing-y-7 border-separate">
									<tbody className=" text-xs leading-5 text-black ">
										...
										<div className="text-black text-sm font-bold" style={{ whiteSpace: "nowrap" }}>
											CLINICAL DIAGNOSIS
										</div>
											...
											{conditionSeverity?.map((item, index) => (
												<tr key={index} className="h-8">
													<td className="border-transparent">
														<div className="text-black text-xs font-normal leading-5">{item.value}</div>
													</td>
												</tr>
											))}
										</div>
