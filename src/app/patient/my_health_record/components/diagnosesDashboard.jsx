	...
	React.useEffect(() => {
		async function fetchFinalDiagnoses() {
			try {
				...
				const allFinalDiagnoses = await getFinalDiagnosisObservations(currentUser.getState().user.id);
			...
			<table className="pt-1.5 leading-5 text-black mt-10 max-w-[100%]">
				...
				<tbody>
					...
					{finalDiagnoses?.map((diagnosis, index) => {
						return (
							<>
								{diagnosis.resource.valueString != null && diagnosis.resource.valueString.length > 0 && (
