...
export default function SocialHistory() {
	...
	const handleSave = async () => {
		...
		const response = await PatientSignUp.updatePatientDetails(patientId, updatedDetails);
		if (response.success) {
	...
	return (
		<>
										...
										{isEditingSocialHistory ? (
											item.variable === "Smoking Status" ? (
												...
												<input
													...
													style={sHistory[0].value === "Non-Smoker" ? disabledInputStyle : {}}
												/>
			...
			{currentPage === 1 && (
				<FamilyHistory currentPage={currentPage} setCurrentPage={setCurrentPage} data={currentFamily} />
			)}

			{currentPage === 2 && <EditFamilyHistory currentPage={currentPage} setCurrentPage={setCurrentPage} />}
		</>
	);
}
