...
export default function ViewSharing() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        (process.env.NEXT_PUBLIC_MIDDLEWARE_API_CALLS ??
          "https://cap-middleware.onrender.com/user") + "/getRequests",
        {
          method: "POST", // or 'PUT'
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...
            requested_from: `${
              currentUser.getState().user.email ?? "testpatient@gmail.com"
            }`,
      ...
      const clean = data?.map(async (item) => {
        ...
        const specialization = await referral.getDoctorSpecialization(
            ...
            <div className="flex flex-col items-stretch w-[82%] ml-5 max-md:w-full max-md:ml-0">
              ...
              <table className="min-w-full divide-y divide-gray-200 mt-10">
                <thead className="bg-gray-50 border border-gray-200 drop-shadow-xl rounded-md">
                  <tr>
                    <th className="px-6 py-3 text-left text-base font-semibold text-black uppercase tracking-wider">
                      Name
                    </th>
                    <th className="px-6 py-3 text-left text-base font-semibold text-black uppercase tracking-wider">
                      Specialization
