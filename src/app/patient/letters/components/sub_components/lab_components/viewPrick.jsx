    ...
    if (observationId) {
      const fetchObservationData = async () => {
        try {
          ...
          // Update ranges
          setRanges(
            observation.resource.rangeQuantity.rangeQuantities.map((range) => ({
              level: range.level,
              min: range.min,
              max: range.max,
            }))
        ...
        <div className="flex flex-col mt-3">
          {ranges.map((range, index) => (
            <div className="flex gap-2 mt-3" key={index}>
              <input
                type="text"
                className="text-sm leading-5 text-black px-1 py-1 rounded  w-16"
              ...
              <input
                type="text"
                className="text-sm text-center leading-5 text-black px-1 py-1 rounded w-12"
                placeholder="Min"
                value={range.min}
                readOnly
              />
            ...
            {labtest?.map((item) => (
              <tr key={item.variable}>
                ...
                <td className="border-l-[16px] border-transparent">
                  <div className="text-black text-sm font-semibold leading-5 self-center my-auto mt-5">
                    {item.variable}
                  </div>
                </td>
                <td className="border-l-[5rem] border-transparent">
                  <div className="text-black text-sm leading-5 ml-auto mt-5">
                    {item.value}
                  </div>
                </td>
              </tr>
