...
export function Prescription({ medicationData, patientData, doctor_id }) {
  ...
  React.useEffect(() => {
    const fetchDoctor = async () => {
      const doctor = await getDoctorByLicense(doctor_id ?? "0029873");
        ...
        <div className="flex flex-col grow shrink-0 basis-0 w-fit max-md:max-w-full">
          <div className="flex flex-col self-center leading-[150%] max-md:max-w-full">
            <div className="self-center text-xl">
              Dr. {medicationData?.resource.requester.agent?.reference ?? ""}
            </div>
        ...
        medicationData?.resource?.medicationData.map(
          (medicationData, index) => (
            ...
            >
              ...
              (
              {medicationData?.resource?.medicationCodeableConcept[0]?.coding[0]
                ?.display ?? ""}
              ) [
              {medicationData?.resource?.dosageInstruction[0]?.doseAndRate[0]
                ?.doseQuantity?.doseUnit ?? ""}
              ]
              <br />
              <br />
              <span className="font-semibold">Sig:</span>{" "}
              {medicationData?.resource?.form.text ?? ""},{" "}
              {medicationData?.resource?.dispenseRequest.dispenseInterval ?? ""}
              , {medicationData?.resource?.note ?? ""}
            </div>
          )
        )}
