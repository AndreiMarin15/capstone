...
import { getMessages, getMessagesAndSubscribe } from "@/app/lib/backend//message/getMessages";
export default function Messaging() {
	const [patient, setPatient] = React.useState("");
	const [message, setMessage] = React.useState("");
	const [changeUser, setChangeUser] = React.useState(false);
	const [newMessage, setNewMessage] = React.useState(false);
	const [chats, setChats] = React.useState([{ id: "", patient: "", patient_full_name: "" }]);
								...
								<div key={index} className="w-full group" onClick={() => getUser(item.id)}>
									<div className="flex gap-5 items-center group-hover:bg-neutral-200">
										...
										<div className="flex flex-col flex-1">
											<div className="text-normal font-semibold whitespace-nowrap">{item.patient_full_name}</div>
										</div>
									</div>
								</div>
							);
						...
						<div className="flex flex-col items-stretch w-[74%] max-md:w-full max-md:ml-0">
							<span className="text-black text-lg font-semibold leading-7 whitespace-nowrap items-stretch grow mt-3 pr-3 pb-5 max-md:mt-10">
								{messageInfo.patients
									? messageInfo.patients.first_name + " " + messageInfo.patients.last_name
									: "undefined"}
							</span>
						</div>
					</div>
