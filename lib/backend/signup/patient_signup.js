...
export const PatientSignUp = {
  ...
  signUpAsPatient: async (userData, patientData) => {
    ...
    if (account.user != null) {
      ...
      if (account.user.id) {
        const addResource = await PUBLIC.insertInto(
          "patient",
          fhirResources.patient
        );
        console.log(addResource);
        const addPerson = await PUBLIC.insertInto(
          "person",
          fhirResources.person
        );
        console.log(addPerson);
        if (addResource === null && addPerson === null) {
          const addAccount = await PUBLIC.insertInto(
            "account",
            fhirResources.account
          );
          console.log(addAccount);
          if (addAccount === null) {
            const addPatient = await PROJECT.insertInto(
              "patients",
              patientInfo
            );
            ...
            await patientData.allergies.forEach(async (allergy) => {
              ...
              await PUBLIC.insertInto("allergyintolerance", aller);
            });
            ...
            await patientData.family_history.forEach(async (family) => {
              ...
              await PUBLIC.insertInto("familymemberhistory", fam);
            });
