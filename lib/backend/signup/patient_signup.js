...
export const PatientSignUp = {
	getDoctors: async () => {
		...
		const toReturn = doctors.filter((doctor) => parseInt(doctor.specialization_id) === 1);
	...
	signUpAsPatient: async (userData, patientData) => {
		...
		if (account.user != null) {
			...
			if (account.user.id) {
				const addResource = await PUBLIC.insertIntoNoSelect("patient", fhirResources.patient);
				console.log(addResource);
				const addPerson = await PUBLIC.insertIntoNoSelect("person", fhirResources.person);
				console.log(addPerson);
				if (addResource === null && addPerson === null) {
					const addAccount = await PUBLIC.insertIntoNoSelect("account", fhirResources.account);
					console.log(addAccount);
					if (addAccount === null) {
						const addPatient = await PROJECT.insertIntoNoSelect("patients", patientInfo);
	...
	retrieveReactions: async (codes) => {
		...
		for (let code of codes) {
			const { data, error } = await supabase.from("icd10").select("*").like("id", `%${code}%`);
