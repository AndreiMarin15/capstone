...
const retrieveReferralData = {
	...
	getFilteredPatients: async () => {
		...
		const p = query.data.filter((patient) => {
			return (
				patient.handled_by?.main_practitioner === currentUser.getState().info.id ||
				...
				.update({
					handled_by: {
						...
						referred_practitioners: patient.data[0].handled_by
							? [...patient.data[0].handled_by.referred_practitioners, referral.data[0].referred_to]
							: [referral.data[0].referred_to],
					},
				})
				.eq("id", referral.data[0].patient_id);

			sendNotification(referral.data[0].patient_id, "Referral", "Your have been referred to a new specialist", referral.data[0].referred_to);
